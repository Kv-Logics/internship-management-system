...
export default function FacultyCard({ fac, internships, setSelectedFaculty, handleDeleteFaculty, user, deletingId }) {
  ...
  return (
    <div onClick={() => setSelectedFaculty(fac)} className="bg-white rounded-none border border-gray-200/80 p-6 shadow-none hover:shadow-none hover:-translate-y-1 transition-all duration-300 cursor-pointer flex flex-col justify-between group">
      <div className="space-y-4">
        <div className="flex justify-between items-start">
          <span className={`inline-flex px-2.5 py-0.5 rounded-none text-[10px] font-bold uppercase tracking-wider ${fac.role === 'dean' ? 'bg-amber-100 text-amber-800' : 'bg-indigo-100 text-indigo-800'}`}>
            {fac.role || 'Faculty'}
          </span>
          
          {fac.email !== 'admin@nitt.edu' && user?.role === 'admin' && (
            <button onClick={(e) => handleDeleteFaculty(fac.faculty_id, e)} disabled={deletingId === fac.faculty_id} className="p-1.5 text-gray-400 hover:text-rose-600 rounded-none hover:bg-rose-50 transition-all opacity-0 group-hover:opacity-100" title="Delete Faculty Account">
              <Trash2 size={15} />
            </button>
          )}
        </div>
