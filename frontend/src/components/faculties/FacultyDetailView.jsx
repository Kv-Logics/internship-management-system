  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center space-x-4">
        <button onClick={() => setSelectedFaculty(null)} className="p-2.5 bg-white hover:bg-gray-50 border border-gray-250 rounded-xl shadow-sm text-gray-700 hover:text-indigo-600 transition-all flex items-center justify-center">
          <ArrowLeft size={18} />
        </button>
        <div>
          <div className="flex items-center space-x-2">
            <h2 className="text-2xl font-bold text-gray-900">{selectedFaculty.faculty_name}</h2>
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider ${selectedFaculty.role === 'dean' ? 'bg-amber-100 text-amber-800' : 'bg-indigo-100 text-indigo-800'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-200/80 shadow-sm">
          ...
          <div className="flex items-center justify-between mt-1">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded font-bold uppercase ${facultyInternships.length >= 5 ? 'bg-rose-100 text-rose-800' : 'bg-emerald-100 text-emerald-800'}`}>
              ...
              <div key={item.internship_id} className="p-6 hover:bg-indigo-50/10 transition-colors">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2"><GraduationCap className="h-5 w-5 text-indigo-600" /><h4 className="text-base font-bold text-gray-900">{item.intern?.intern_name}</h4><span className={`text-[10px] px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider ${new Date(item.end_date) < new Date() ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'}`}>{new Date(item.end_date) < new Date() ? 'Completed' : 'Ongoing'}</span></div>
                  ...
                  <div className="flex items-center gap-3 flex-wrap md:justify-end">
                    {(item.certificate || new Date(item.end_date) < new Date() || user?.role === 'admin') && (<button onClick={(e) => handlePreviewCertificate(item, e)} className="flex items-center space-x-1.5 px-3.5 py-2 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-xl text-xs font-bold hover:bg-emerald-100 transition-all"><Eye size={14} /><span>Preview Cert</span></button>)}
                    <button onClick={() => handleDeleteInternship(item.internship_id)} className="p-2 text-rose-600 hover:bg-rose-50 border border-transparent hover:border-rose-200 rounded-xl transition-all" title="Delete Internship Record"><Trash2 size={16} /></button>
                  </div>
                </div>
              </div>
            ))
