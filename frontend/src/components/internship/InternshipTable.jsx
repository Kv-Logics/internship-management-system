                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                          <span className={`inline-flex text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${status === 'not_started' ? 'bg-slate-100 text-slate-700' : status === 'ongoing' ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'}`}>
                            {status === 'not_started' ? 'Not Started' : status === 'ongoing' ? 'Ongoing' : 'Completed'}
                          </span>
                          
                          {/* Payment status badge */}
                          {item.is_paid ? (
                            <span className="inline-flex text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider bg-emerald-50 text-emerald-750 border border-emerald-150">
                              Paid
                            </span>
                          ) : item.transaction_number ? (
                            <span className="inline-flex text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider bg-amber-50 text-amber-750 border border-amber-150">
                              Pending
                            </span>
                          ) : (
                            <span className="inline-flex text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider bg-rose-50 text-rose-750 border border-rose-150">
                              Unpaid
                            </span>
                          )}
                        </div>
                        ...
                        {!item.is_paid && user?.role === 'faculty' && (
                          <div className="mt-2">
                            <button
                              ...
                              className="inline-flex items-center space-x-1 px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-700 rounded-lg text-[10px] font-bold transition-all cursor-pointer"
                            >
                        ...
                        {item.remarks && item.remarks.includes("Payment Declined:") && (
                          <div className="text-[10px] text-rose-700 bg-rose-50 border border-rose-100 p-1.5 px-2 rounded-xl mt-2 leading-relaxed flex items-start max-w-[240px] whitespace-normal">
                            ...
                            <div className="flex items-center space-x-1.5">
                              {reportDoc && (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="p-1.5 text-emerald-655 hover:bg-emerald-50 border border-transparent hover:border-emerald-100 rounded-lg transition-all"
                                  title="Download/View Report"
                                >
                                  <CheckCircle size={15} />
                                </a>
                              )}
                              <label className="p-1.5 text-gray-500 hover:text-purple-600 hover:bg-purple-50 border border-transparent hover:border-purple-100 rounded-lg transition-all cursor-pointer" title={reportDoc ? "Re-upload Report" : "Upload Report"}>
                                <Upload size={15} />
                                <input
                                  type="file"
                                  accept=".pdf,.doc,.docx"
