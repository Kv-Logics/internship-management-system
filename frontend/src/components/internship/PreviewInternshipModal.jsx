      ...
      <div className="bg-white rounded-3xl w-full max-w-2xl border border-gray-200 overflow-hidden shadow-2xl animate-scaleUp">
        ...
        <div className="p-6 space-y-6 overflow-y-auto max-h-[70vh]">
          ...
          <div className="border-t border-gray-150 pt-4 space-y-3">
            ...
            <div className="bg-gray-50 border border-gray-200 p-4 rounded-2xl flex flex-col space-y-3">
              <div className="flex items-center space-x-8">
                <div>
                  <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Status</span>
                  {previewItem.is_paid ? (
                    <span className="inline-flex text-xs font-bold text-emerald-700 bg-emerald-50 border border-emerald-150 p-1 px-2.5 rounded-lg mt-1">Verified Paid</span>
                  ) : previewItem.transaction_number ? (
                    <span className="inline-flex text-xs font-bold text-amber-700 bg-amber-50 border border-amber-150 p-1 px-2.5 rounded-lg mt-1">Verification Pending</span>
                  ) : (
                    <span className="inline-flex text-xs font-bold text-rose-700 bg-rose-50 border border-rose-150 p-1 px-2.5 rounded-lg mt-1">Unpaid</span>
                  )}
                </div>
                <div>
                ...
                {!previewItem.is_paid && user?.role === 'faculty' && (
                  <div className="flex-1 text-right">
                    <button
                      ...
                      className="px-4 py-2 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-700 rounded-xl text-xs font-bold transition-all cursor-pointer"
                    >
              ...
              {previewItem.remarks && previewItem.remarks.includes("Payment Declined:") && (
                <div className="border-t border-rose-100 pt-3 mt-1 flex items-start text-xs text-rose-700 bg-rose-50/50 p-2.5 rounded-xl border border-rose-100">
          ...
          <div className="border-t border-gray-150 pt-4 space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-gray-50 border border-gray-200 p-4 rounded-2xl gap-4">
              ...
              <div>
                <label className="px-4 py-2 bg-purple-50 text-purple-700 hover:bg-purple-100 border border-purple-200 rounded-xl text-xs font-bold shadow-sm cursor-pointer transition-all flex items-center space-x-1.5">
